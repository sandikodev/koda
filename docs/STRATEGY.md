...
### Phase 1: Institutionalization (Current)
- [x] Separate framework code into Bun Workspace.
- [x] Rename and Brand as **Koda Core**.
- [x] Extract Security Middleware to `@koda/core/server` (Implemented via `koda.security()` primitive).
- [ ] Extract Design System primitives to `@koda/core/ui`.
...
- [ ] **Koda CLI**: `npx create-koda-app` and local scaffolding commands.
- [ ] **Forensic UI**: Visual inspector inside the premium error page to bridge the gap between error and fix.
