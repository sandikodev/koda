# 🛰️ Koda Zenith: The Blueprint Docs

> *"Imagine the perfect fullstack framework. This is what Koda looks like."*

This directory contains the modular blueprint for the Koda framework.

## 📚 Table of Contents

### Phase 1: The Standard Chassis (Concrete)
*   [00_manifesto.md](./00_manifesto.md) - Vision & Core Philosophy.
*   [01_project_structure.md](./01_project_structure.md) - The Institutional Anatomy.
*   [02_configuration.md](./02_configuration.md) - `koda.config.ts`.
*   [03_core_concepts.md](./03_core_concepts.md) - General Concepts.

#### 🧠 Deep Architecture Dives (Must Read)
*   **[🎨 Zenith UI Vision](./zenith_ui_vision.md)** - The Declarative Frontend Engine (.koda).
*   **[⚙️ Agnostic Kernel Vision](./agnostic_kernel_vision.md)** - The Backend Engine & Adapters.
*   [04_foundation_features.md](./04_foundation_features.md) - Auth, DB, Env, Testing.
*   [05_gateway_lifecycle.md](./05_gateway_lifecycle.md) - Proxy & Edge Lifecycle.
*   [06_data_and_services.md](./06_data_and_services.md) - Content, Uploads, Jobs, Email, Payments.
*   [07_extended_features.md](./07_extended_features.md) - Analytics, PWA, Search, Forms, Admin.
*   [08_devops_dx.md](./08_devops_dx.md) - Migrations, Logging, CI/CD, Forensic DX, Security.

### Bridging the Gap
*   [09_scalable_spectrum.md](./09_scalable_spectrum.md) - Functional Mode vs. Enterprise Mode.
*   [10_philosophy_and_design.md](./10_philosophy_and_design.md) - Architecture & Edge Philosophy.

### Phase 2 & 3: The Future (Experimental)
*   [11_future_horizons.md](./11_future_horizons.md) - Roadmap, Event Horizon, Singularity.
*   [12_cultural_engine.md](./12_cultural_engine.md) - The Cultural Impact.

---
*Original Monolithic Document: [`_FULLSTACK_VISION_OLD.md`](./_FULLSTACK_VISION_OLD.md)*
